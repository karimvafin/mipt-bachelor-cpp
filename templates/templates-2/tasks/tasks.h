...
public:
    ...
    template <typename U>
    U convert_to() const {
        return static_cast<U>(value_convert);
    }
};
...
public:
    ...
    Pair(const Pair<T2, U2>& other) {
        first_value=other.first();
        second_value=other.second();
    }
...
auto sum_all(const Args&... args) {
    if constexpr (sizeof...(Args) == 0) {
        ...
        using together_type= std::common_type_t<Args...>;
        return static_cast<together_type>(0) + (args + ...);
    }
}
...
int count_if_positive(const Args&... args) {
    if constexpr (sizeof...(Args) == 0) {
        ...
        using together_type= std::common_type_t<Args...>;
        return (0 + ... + static_cast<together_type>(args > 0));
    }
}
