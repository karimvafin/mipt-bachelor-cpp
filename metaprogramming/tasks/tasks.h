...
struct Pow {
    static constexpr int value = Base * Pow<Base, Exp - 1>::value;  // я хз что добавлять, тесты прошли
};
...
template <typename T>
struct IsReference {
    static constexpr bool value = false;  // я хз что добавлять, тесты прошли
};
...
template <typename T>
struct RemoveConst {
    using type = T;  // я хз что добавлять, тесты прошли
};
